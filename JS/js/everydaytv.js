function getAddress(field) {
    ...
    var str = res.match(/<iframe src="(.*?)"/);
    if (str == null || str.length != 2) {
        return res;
    }
    object = { url: "https://everydaytv.top/" + str[1], header: "" };
    res = get.call(object);
    var destr = res.match(/str=de\("(.*?)"\)/);
    var replacestr = res.match(/replace\(\/(.*?)\/g/);

    if (destr.length != 2 || replacestr.length != 2) {
        return res;
    }

    str = de(destr[1]).replace(new RegExp(replacestr[1], 'g'), "M");
    var base = new Base64();
    return JSON.stringify({ url: base.decode(str) });
}
